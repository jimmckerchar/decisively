...
```ruby
Decisive.choice(text, options: %w[billing bug account])     # => Decision(value: "billing", confidence: 0.91, ...)
Decisive.bool(text, statement: "This message is spam.")     # => Decision(value: true, ...)
...
Decisive.warm! unless Rails.env.test?
...
```ruby
Decisive.calibrate!(examples)  # => { temperature: 1.85, ece_before: 0.21, ece_after: 0.08 }
```
...
- Laya is a trained 421M decision model that scores all options in one forward pass.
  Decisive uses an off-the-shelf NLI cross-encoder, which runs one pass *per option*,
...
`Decisive::Decidable` can be used without Rails: `require "decisive/decidable"` (needs `activesupport`).
